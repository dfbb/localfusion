# P1-T11 latency_samples 写入 + speed 查询

**阶段:** 1 基础层 · **前置:** P1-T05 · 见全局约束: `00-index.md`

**Goal:** 延迟样本写入 + 最近 N 条平均吞吐子查询（设计 §4，避免 AVG+LIMIT 误用）+ 陈旧模型探测查询。

**Files:** Modify: `src/db/latency.rs`

**Produces:** `Db::{latency_insert,latency_avg_recent,latency_models_without_recent}`。
...
- [ ] **Step 2: 运行确认失败** — Run: `cargo test --lib db::latency` → FAIL。
...
- [ ] **Step 4: 运行确认通过** — Run: `cargo test --lib db::latency` → PASS。

- [ ] **Step 5: 提交**

```bash
git add src/db/latency.rs
git commit -m "feat: latency_samples 写入 + 最近N条平均吞吐子查询 + 陈旧模型探测查询"
```
