# P2-T05 ResponsesConnector

**阶段:** 2 Connector · **前置:** P2-T01, P2-T02 · 见全局约束: `00-index.md`

**Goal:** OpenAI Responses 出口（自研，llm-switch 无对应，设计 §6.2）。请求 `{model,input:[{role,content:[{type:input_text,text}]}],max_output_tokens}`；非流式 `output:[{type:message,content:[{type:output_text,text}]}]` + `usage`；SSE `response.output_text.delta`/`response.completed`。

**Files:** Modify: `src/connector/responses.rs`

**Produces:** `ResponsesConnector` 实现 `Connector`；`build_responses_request`、`parse_responses_response`、`ResponsesSseState`。
...
mod tests {
    ...
    use crate::unified::*;
    fn ctx() -> EgressCtx {
        EgressCtx { base_url:"u".into(), model:"gpt-5".into(), auth: AuthKind::Bearer,
    ...
    fn req() -> UnifiedRequest {
        UnifiedRequest { items: vec![Item::Message { role: Role::User, content: vec![ContentBlock::Text("hi".into())] }],
            tools: vec![], max_tokens: Some(40), temperature: None, stream: false, raw_extra: serde_json::Value::Null }
    }
    #[test]
    ...
    fn parse_response_output_text_and_usage() {
        ...
        match &r.items[0] { Item::Message{content,..} => match &content[0] {
            ContentBlock::Text(t)=>assert_eq!(t,"答"), _=>panic!() }, _=>panic!() }
    }
    #[test]
    fn sse_delta_and_completed() {
        let mut st = ResponsesSseState::new("gpt-5".into());
        let e = st.push(&serde_json::json!({"type":"response.output_text.delta","delta":"你"})).unwrap();
        assert!(matches!(e[0], UnifiedStreamEvent::TextDelta { .. }));
        st.push(&serde_json::json!({"type":"response.completed","response":{"usage":{"input_tokens":1,"output_tokens":5}}})).unwrap();
        let fin = st.finish();
        assert!(fin.iter().any(|e| matches!(e, UnifiedStreamEvent::Done { call: Some(c), .. } if c.output_tokens==5)));
    }
}
```

- [ ] **Step 2: 运行确认失败** — Run: `cargo test --lib connector::responses` → FAIL。
    ...
    for item in &req.items {
        if let Item::Message { role, content } = item {
            let role_s = match role { Role::System => "system", Role::User => "user",
                Role::Assistant => "assistant", Role::Tool => "user" };
            let text: String = content.iter().filter_map(|c| match c {
                ContentBlock::Text(t) => Some(t.clone()), _ => None }).collect();
            input.push(json!({"role": role_s, "content":[{"type":"input_text","text":text}]}));
        }
    }
    let mut body = json!({"model": ctx.model, "input": input, "stream": req.stream});
    if let Some(mt) = req.max_tokens.or(ctx.default_max_tokens) { body["max_output_tokens"] = json!(mt); }
            ...
            if let Some(content) = item.get("content").and_then(|v| v.as_array()) {
                for block in content {
                    if block.get("type").and_then(|t| t.as_str()) == Some("output_text") {
                        if let Some(t) = block.get("text").and_then(|v| v.as_str()) { text.push_str(t); }
                    }
                }
            }
        }
...
- [ ] **Step 4: 实现（ResponsesSseState + Connector impl）**

```rust
pub(super) struct ResponsesSseState {
    model_id: String, text: String, input_tokens: u64, output_tokens: u64, has_usage: bool,
}
impl ResponsesSseState {
    pub(super) fn new(model_id: String) -> Self {
        Self { model_id, text: String::new(), input_tokens: 0, output_tokens: 0, has_usage: false }
    }
}
impl SseTranslator for ResponsesSseState {
        ...
        match ty {
            "response.output_text.delta" => {
                if let Some(d) = evt.get("delta").and_then(|v| v.as_str()) {
                    if !d.is_empty() { self.text.push_str(d); out.push(UnifiedStreamEvent::TextDelta { text: d.into() }); }
                }
            }
            ...
            "error" | "response.failed" => return Err(ConnError::HardFail(format!("responses sse error: {evt}"))),
            _ => {}
        }
        Ok(out)
    }
    fn finish(&mut self) -> Vec<UnifiedStreamEvent> {
        ...
        vec![UnifiedStreamEvent::Done { usage, call: Some(call), finish_reason: None }]
    }
}
...
impl Connector for ResponsesConnector {
    ...
    async fn stream(&self, req: &UnifiedRequest, ctx: &EgressCtx) -> Result<UnifiedStream, ConnError> {
        ...
        run_egress(url, headers, body, ctx.http.clone(),
            Box::new(ResponsesSseState::new(ctx.model.clone())), ctx.model.clone()).await
    }
}
```
...
```bash
cargo test --lib connector::responses && cargo test && cargo clippy --all-targets
git add src/connector/responses.rs
git commit -m "feat: ResponsesConnector(input数组/output解析/response.* SSE)"
```

> **阶段 2 完成**：三连接器可把 UnifiedRequest 打到供应商并解析流/非流。
