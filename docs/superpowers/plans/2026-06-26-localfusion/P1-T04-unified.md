...
**Goal:** 定义协议无关的核心类型（设计 §6.1）：请求/响应/Item、用量、流式事件、ConnError、CallRecorder、StrategyTrace。

**Files:** Modify: `src/unified.rs`

**Produces（后续所有层依赖，签名须精确）:**
- `UnifiedRequest{items,tools,max_tokens,temperature,stream,raw_extra}`
- `enum Item { Message{role,content}, Reasoning{content}, ToolCall{id,name,args}, ToolResult{id,content} }`
- `enum Role{System,User,Assistant,Tool}`、`enum ContentBlock{Text(String),Image{url}}`
- `ToolDef{name,description,parameters,builtin:Option<String>}`、`Usage{input_tokens,output_tokens}`
- `enum CallRole{Member,Judge,Tool}`、`enum CallStatus{Ok,Failed}`
- `ModelUsage{model_id,role,input_tokens,output_tokens,cost,status,estimated,latency_secs}`
- `UnifiedResponse{items,usage,model_id,calls}`
- `enum UnifiedStreamEvent{Started{model_id},TextDelta{text},ReasoningDelta{text},ToolCall{id,name,args},Done{usage,call:Option<ModelUsage>,finish_reason},Error{message,call:Option<ModelUsage>}}`
- `UnifiedStream{rx,upstream_request_id}`
- `enum ConnError{HardFail(String),Http(String)}` + `From<ConnError> for FusionError`
- `CallRecorder`（`record/drain`）、`StrategyTrace`（`set_status/add_member_answer/set_judge/add_candidate/add_attempt/add_turn/snapshot`）
...
mod tests {
    ...
    fn usage(m: &str, st: CallStatus) -> ModelUsage {
        ModelUsage { model_id: m.into(), role: CallRole::Member, input_tokens: 1,
            output_tokens: 2, cost: 0.0, status: st, estimated: false, latency_secs: 0.1 }
    }
    #[test]
...
- [ ] **Step 2: 运行确认失败** — Run: `cargo test --lib unified` → FAIL。
...
pub enum ContentBlock { Text(String), Image { url: String } }
...
- [ ] **Step 5: 实现（CallRecorder + StrategyTrace）**

```rust
#[derive(Default)]
pub struct CallRecorder { calls: Mutex<Vec<ModelUsage>> }
...
#[derive(Default)]
pub struct StrategyTrace { data: Mutex<TraceData> }

impl StrategyTrace {
    pub fn set_status(&self, status: &str) { self.data.lock().unwrap().status = Some(status.into()); }
    ...
    pub fn add_candidate(&self, model_id: &str, metric: serde_json::Value) {
        self.data.lock().unwrap().candidates.push(serde_json::json!({ "model_id": model_id, "metric": metric }));
    }
    pub fn add_attempt(&self, model_id: &str, ok: bool, error: Option<&str>) {
        self.data.lock().unwrap().attempts.push(serde_json::json!({ "model_id": model_id, "ok": ok, "error": error }));
    }
    pub fn add_turn(&self, turn: serde_json::Value) { self.data.lock().unwrap().turns.push(turn); }
    pub fn snapshot(&self) -> serde_json::Value {
        let d = self.data.lock().unwrap();
        serde_json::json!({
            "status": d.status, "member_answers": d.member_answers, "judge": d.judge,
            "candidates": d.candidates, "attempts": d.attempts, "turns": d.turns })
    }
}
```

- [ ] **Step 6: 运行确认通过** — Run: `cargo test --lib unified` → PASS（3 个）。
...
git commit -m "feat: 统一中间类型 + CallRecorder + StrategyTrace + ConnError"
```
