# P4-T03 OpenAI Responses 入口翻译

**阶段:** 4 装配 · **前置:** P4-T02 · 见全局约束: `00-index.md`

**Goal:** Responses body ↔ Unified（`input` 可为 string 或数组，设计 §2）。

**Files:** Modify: `src/ingress/openai_responses.rs`
...
- [ ] **Step 2: 运行确认失败** — Run: `cargo test --lib ingress::openai_responses` → FAIL。

- [ ] **Step 3: 实现 openai_responses.rs**
...
pub fn parse_request(body: &Value) -> Result<UnifiedRequest, FusionError> {
    ...
    match body.get("input") {
        Some(Value::String(s)) =>
            items.push(Item::Message { role: Role::User, content: vec![ContentBlock::Text(s.clone())] }),
        Some(Value::Array(arr)) => {
            for it in arr {
                ...
                let text = it.get("content").and_then(|c| c.as_array()).map(|blocks| {
                    blocks.iter().filter_map(|b| b.get("text").and_then(|t| t.as_str()).map(String::from)).collect::<String>()
                }).or_else(|| it.get("content").and_then(|v| v.as_str()).map(String::from)).unwrap_or_default();
...
pub fn format_response(resp: &UnifiedResponse) -> Value {
    json!({
        "id": "resp-localfusion", "object": "response", "model": resp.model_id, "status": "completed",
        "output": [{"type":"message","role":"assistant","content":[{"type":"output_text","text":answer_text(resp)}]}],
    ...
    match ev {
        ...
        UnifiedStreamEvent::Done { usage, .. } => vec![json!({
            ...
            "response":{"status":"completed","usage":{
                "input_tokens":usage.input_tokens,"output_tokens":usage.output_tokens}}}).to_string()],
        UnifiedStreamEvent::Error { message, .. } =>
            vec![json!({"type":"response.failed","response":{"error":{"message":message}}}).to_string()],
        _ => vec![],
    }
}
```
...
```bash
cargo test --lib ingress::openai_responses && cargo clippy --all-targets
git add src/ingress/openai_responses.rs
git commit -m "feat: OpenAI Responses 入口(input string/array + output + response.* SSE)"
```
