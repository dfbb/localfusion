...
通用约定：所有页面经 `lib/api.ts` 调 §13.1 的 REST API，自动注入
`Authorization: Bearer <admin-token>`；401 → 跳登录页。数据获取用 TanStack Query
（缓存 + 失效重取），表单用 react-hook-form + zod 校验，提示用 shadcn `toast`，
确认删除用 `AlertDialog`。整体布局：左侧固定侧边栏（5 个导航项 + 登出），右侧内容区。

#### 13.2.0 登录页 `Login.tsx`
- 单输入框（admin token，password 型）+「登录」按钮。
- 提交：以该 token 调 `GET /admin/api/health`；200 则存入内存 + sessionStorage 并跳
  Models 页，401 则 toast「token 无效」。
- 已登录（sessionStorage 有 token）直接跳过登录页。

#### 13.2.1 模型/策略配置（两个子页）

**A. 真实模型 `Models.tsx`** — 数据源 `GET /admin/api/models`
- 表格列：id、connector（chat/anthropic/responses 彩色 badge）、base_url、model、
  密钥状态（api_key_enc 有值显示绿点「已加密存储」/ api_key_env 显示「env: NAME」/
  都无显示红点「未配置」）、操作（编辑 / 删除）。
- 「新建模型」按钮 → 弹出 `Dialog` 表单，字段：
  - `id`（文本，必填，唯一，创建后不可改）
  - `connector`（下拉：chat | anthropic | responses，必填）
  - `base_url`（文本，必填，URL 校验）
  - 密钥方式（单选）：直填 api_key（password 输入框）/ 指定 api_key_env（文本）
  - `model`（文本，必填，发给供应商的真实模型名）
  - `anthropic_version`（仅 connector=anthropic 时显示，默认 2023-06-01）
  - `extra`（可选，JSON 编辑器，连接器私有字段如 default_max_tokens）
  - 提交 → `POST /admin/api/models`；api_key 一旦提交即加密存储，响应不回显。
- 编辑：复用表单，api_key 字段显示占位「已设置（留空则不变）」；留空时 PUT 不改密钥。
- 删除：若该模型被任一虚拟模型引用为 member 或 judge，后端返回 409 + 引用列表，前端
  toast 阻止删除并列出引用方。

**B. 虚拟模型 `VirtualModels.tsx`** — 数据源 `GET /admin/api/virtual-models`
- 表格列：name、strategy（badge）、成员数、操作。
- 「新建虚拟模型」→ 表单：
  - `name`（文本，必填，唯一，即对外模型名）
  - `strategy`（下拉，选项来自 `GET /admin/api/strategies`）
  - **成员列表**：可拖拽排序（dnd-kit），每行从「已有真实模型」下拉选一个；顺序即
    position。failover/speed/cheapest 语义依赖顺序，UI 标注「顺序 = 优先级」；
    multimodal 仅取首个为主模型，UI 标注「members[0] 为主推理模型」。
  - **策略参数动态表单**：`GET /admin/api/strategies` 对每个策略返回一份 JSON Schema
    （params schema），前端据 schema 渲染对应控件：
    - `synthesize` / `best-of-n`：`judge`（真实模型下拉，必填）、`min_answers`（数字）、
      `strict`（开关）
    - `failover`：`timeout_secs`（数字）
    - `speed`：`explore`（开关，是否给无样本模型机会）、探测间隔（数字，分钟）
    - `cheapest`：`tokenizer`（下拉：approx | tiktoken）、输出估算上限（数字）
    - `multimodal`：能力路由表——`web_search` / `image_generation` / `tool_search` /
      `image_query` 各一个真实模型下拉（可留空表示不支持该能力）、`max_iterations`（数字）
  - 提交 → `POST /admin/api/virtual-models`，body 含 strategy + params(JSON) +
    members(有序数组)。
- 编辑/删除同 A 的模式。

#### 13.2.2 密钥/ACL 管理 `Keys.tsx` — 数据源 `GET /admin/api/keys`
- 表格列：label、状态（enabled 开关，切换调 PUT）、创建时间、ACL 摘要（显示
  `*` 或前 N 个虚拟模型名 +「…」）、操作（编辑 ACL / 删除）。绝不显示明文。
- 「新建 key」→ 输入 label → `POST /admin/api/keys` → **弹窗一次性展示明文 key**，
  附「复制」按钮 + 红色提示「关闭后无法再次查看」，关闭后列表刷新。
- 「编辑 ACL」→ 弹窗：单选「允许全部（`*`）」或「指定白名单」；选指定时多选框列出所有
  虚拟模型名（来自 virtual-models）。保存 → `PUT /admin/api/keys/:id/acl`。
- 删除：`AlertDialog` 确认 → `DELETE /admin/api/keys/:id`。

#### 13.2.3 状态/监控面板 `Monitor.tsx`
顶部时间范围选择器（小时 / 天 / 周 + 自定义起止），下分四区：
- **总用量卡片**（`GET /admin/api/stats/usage/summary`）：累计请求数、累计 input/output/
  total token、累计成本（缺价格部分标注「不含未定价模型」）。
- **用量趋势 + 排行**（`GET /admin/api/stats/usage?scope=&granularity=`）：
  - 折线图：按所选粒度的 total token / 成本时间序列（scope=total）。
  - 切换标签 real / virtual：表格排行（按 total_token 降序）列出各真实模型 / 各虚拟
    模型的 requests、input/output/total token、cost、errors；行可点开看其单独趋势。
- **吞吐延迟**（`GET /admin/api/stats/latency?model=&window=`）：每个真实模型最近吞吐
  （tokens/s）折线图，探测样本（is_probe=1）用不同标记点区分真实请求。
- **价格表**（`GET /admin/api/stats/prices`）：只读表格，model_id / price_in / price_out
  / updated_at，updated_at 超过阈值（如 7 天）标黄「价格可能过期」。
- **日志配置区块**（`GET/PUT /admin/api/settings/logging`）：level 下拉（debug/info/
  error，保存即热重载）、log_file 文本框（保存提示「需重启生效」）、log_to_stdout 开关。
- **request_log 查询**（`GET /admin/api/stats/requests`）：可按 virtual_name / strategy /
  status / 时间筛选的明细表格，分页。

#### 13.2.4 在线调试 / playground `Playground.tsx`
- 表单：虚拟模型下拉（来自 virtual-models）+ 多行 prompt 输入 +「发送」按钮。
- 提交 → `POST /admin/api/playground`（body: virtual_name + prompt），后端实际跑一次
  ...
  ```
- 展示分区按策略类型渲染 `detail`：
  - **panel 类（synthesize/best-of-n）**：左侧并排显示各成员答案卡片（model + 答案 +
    ok/fail + 用量），右侧显示 judge 的合成/选优结果；顶部显示 diversity 分级 badge
    （full / degraded / stop）。
  - **单模型类（failover/speed/cheapest）**：显示「被选中模型」+ 选择理由——speed 显示
    各候选最近吞吐对比、cheapest 显示各候选成本估算对比、failover 显示尝试链（哪些
    失败、最终哪个成功）。
...
│  ├─ lib/api.ts          # 封装 admin API + token 注入 + 401 跳登录
│  ├─ components/ui/      # shadcn 组件
│  ├─ pages/
│  │  ├─ Login.tsx  Models.tsx  VirtualModels.tsx
│  │  ├─ Keys.tsx   Monitor.tsx  Playground.tsx
│  └─ hooks/  types/
└─ dist/                  # build 产物 → rust-embed 嵌入
```

前端依赖：react、react-router、@tanstack/react-query、react-hook-form、zod、
@dnd-kit（成员拖拽排序）、recharts（趋势图）、tailwind + shadcn/ui。

构建：`cd web && npm install && npm run build`，再 `cargo build --release`。
CI 两步串联。`rust-embed` 在 `dist/` 缺失时回退到一个占位页（仅显示「前端未构建」），
保证 Rust 核心可独立编译。
