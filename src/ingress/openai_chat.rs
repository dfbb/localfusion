...
/// Extract the content of an OpenAI Chat message: return string content directly; for array-typed (multi-part/multimodal) content, concatenate all text segments
...
/// Parse an OpenAI Chat Completions request body into a UnifiedRequest
...
/// Extract the first assistant text from a UnifiedResponse
...
/// Format a UnifiedResponse into an OpenAI Chat Completions response body
...
/// Format an error body per the OpenAI protocol (non-streaming)
pub fn format_error(message: &str) -> Value {
    json!({"error": {"message": message, "type": "invalid_request_error"}})
}

/// Convert a UnifiedStreamEvent into a list of OpenAI SSE event strings
...
mod tests {
    ...
    fn parse_array_content() {
        // OpenAI multi-part/multimodal content: concatenate all text segments, ignore non-text blocks
        let body = serde_json::json!({"model":"vm","messages":[
            {"role":"user","content":[
